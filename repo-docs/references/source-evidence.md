...
| Resume uses a partial current observation without making strict identity CP optional. | [`ResumeObservation`](../../src/pokemon_go_cleanup/batch.py) holds reliable name, optional CP/HP, fingerprint, and wide nickname only inside `_prepare_resume_position()`. Two agreeing name readers can prove the current page differs before CP; same-name CP misses get two CP-only retries whose static ROI must remain unchanged. A total miss may match the last row only through generic/wide name, exact HP, and the existing distance-8 fingerprint, including saved expected nickname for verified rows. The no-CP switch path permits one gesture only. `SummaryIdentity.cp`, `_same_switch_identity`, duplicate, rename, wrap, and append rules are unchanged. | Source-, test-, and saved-real-image-confirmed | Focused tests cover different-name/no-CP/no-swipe continuation, retry recovery, strong fallback success, HP mismatch, verified nickname mismatch, and zero-input aborts. Real `0826.csv + resume_current.png` replay validates the stored last row, recognizes current 索財靈 as different, sends no swipe, and becomes ready to scan. | Batch walkthrough |
| Batch CSV is durable before switching, and switching is bounded by identity checks. | [`BatchScanService`](../../src/pokemon_go_cleanup/batch.py) appends through `atomic_write_text` immediately after a complete one-scan result, then requires a matching post-exit summary before the configured left gestures: `(1180,1500)` to `(260,1500)` over 600 ms, then `(1300,1500)` to `(140,1500)` over 850 ms. Normal rows require the unchanged strict name/CP/fingerprint precheck. Verified renamed rows with missing CP take two static-fingerprint-guarded CP-only retries; only a total miss with exact saved nickname, exact HP, and distance-8 fingerprint may send the first gesture, never the retry. [`test_batch.py`](../../tests/test_batch.py) asserts strict recovery and all no-input negatives. | Source-, test-, and saved-real-image-confirmed | The direction is operator-confirmed for the fixed Mate 30 profile; no right-swipe fallback is implemented. The real gold-frame `before_switch.png` for 索財靈 matches `索財靈10/15/15`, `73/73HP`, and fingerprint distance 0 against its blue saved baseline. | Batch walkthrough |
| Nickname mutation is opt-in and batch-safe through dedicated transitions. | `--rename-with-iv` in [`cli.py`](../../src/pokemon_go_cleanup/cli.py) enables [`AutoScanService`](../../src/pokemon_go_cleanup/automation.py) only after IV recognition. Both editor openings require `detail_summary` and tap the fixed nickname-row center `(720,1460)`. After reset, generic summary OCR remains only a page-state gate; the separate `(150,1300,1290,1550)` reader joins left-to-right name tokens and supplies the default nickname used to build the expected result. An empty wide name stops before the second editor/input. Exact width-sensitive editor/final text, CP, optional HP, and unchanged distance-8 fingerprint gate persistence. [`BatchScanService`](../../src/pokemon_go_cleanup/batch.py) additionally wraps a verified rename batch only when the switched screenshot's wide nickname equals the first row's saved nickname and its CP/HP/fingerprint match. Its separate verified pre-switch CP-missing path requires the same expected wide nickname plus exact baseline HP and distance-8 fingerprint after two CP-only retries. `_same_switch_identity` remains unchanged. Tests cover a one-character generic fragment paired with a complete wide default name, wrong editor text, the fixed tap boundary, nickname/CP/HP/fingerprint negatives, durable rows, renamed resume, and one-swipe fallback. | Source-, synthetic-test-, and saved-real-image-confirmed | The saved `怒鸚哥` frame produces split `怒`/`鸚哥` candidates in two generic variants but the wide reader returns `怒鸚哥`; generic one-character evidence can no longer construct `怒11/7/8`. Real row 1/6 replay also keeps general wrap false while the dedicated wide check returns true for `飄飄球1225` versus expected `飄飄球12/2/5` with identical CP/fingerprint. The latest gold 索財靈 frame satisfies all three dedicated pre-switch fallback signals. | Automatic and batch walkthroughs |
