...
| A bright or saturated event background cannot by itself invalidate a real detail summary. | [`HuaweiMate30PageDetector`](../../src/pokemon_go_cleanup/automation.py) accepts name+CP or name+fixed-ROI HP and records `summary_evidence`. It runs the three ordinary CP/name variants first, then one HSV near-white mask, then the six threshold variants, and HP only after every CP path fails. HSV requires a CP-band prefixed candidate or a confidence-0.95 bare digit candidate with the wide/tall geometry of the complete CP label; isolated background digits and low-confidence `000` are rejected. [`RecognitionService`](../../src/pokemon_go_cleanup/recognition.py) uses the same early-stop order. | Source-, test-, and saved-real-image-confirmed | Known ordinary summaries retain their values, including `古月鳥` CP1217. Real `resume_current.png` for 索財靈 previously produced only `O`/`000`; HSV now yields synthetic reliable candidate `CP458`, while separate fixed-ROI OCR retains `73/73HP`. The committed regression is skipped only when the ignored local screenshot is absent. | Automatic walkthrough |
...
| Appraisal requires OCR `調查寶可夢`, positive entry evidence, and IV bars. | [`HuaweiMate30PageDetector`](../../src/pokemon_go_cleanup/automation.py) derives and safety-checks the menu target and reuses the IV detector. After the menu-row click it accepts direct IV geometry or high-confidence `你好` from the fixed bottom-left `(80,2450,420,2615)` ROI as the explicit dialogue-entry state; only then does the service immediately send its one existing dialogue-advance tap. The wider dialogue detector and bounded menu-disappearance fallback remain fail-safe alternatives. Focused tests cover exact text and confidence. | Source-, test-, and saved-real-image-confirmed | The first saved entry frame from scan `20260826_123322_191081_f91f29908297412f8f2232bc650c4945` returned `你好`, confidence 0.99997, and `appraisal_dialogue` in 0.50 seconds. The older generic detector missed this visible greeting and the live step took 13.39 seconds. This is specific to the current Mate 30 Traditional Chinese layout. | Automatic walkthrough |
...
| Every automatic transition is target-state-driven rather than whole-screen-stability-driven. | Moves polls for `detail_moves`; menu polls for `action_menu`; appraisal entry/bars, nickname keyboard/dialog/summary, and appraisal exit each poll their explicit approved states. The unreachable `_tap()`/`_wait()` chain, full-screen pixel-difference utility, stable-wait configuration, and debug artifact writer were removed. Timing JSON records `stable_wait_seconds: 0` per step and in total. | Source- and test-confirmed | Latest pre-change profiles already contained no stability artifacts; their long steps came from detector OCR, including 13.39 seconds for appraisal entry and 11.60 seconds for appraisal exit. OCR is now measured separately rather than misattributed to stability. | Automatic walkthrough |
...
| Nickname mutation is opt-in and batch-safe through dedicated transitions. | `--rename-with-iv` in [`cli.py`](../../src/pokemon_go_cleanup/cli.py) enables [`AutoScanService`](../../src/pokemon_go_cleanup/automation.py) only after IV recognition. Both editor openings require `detail_summary` and tap the fixed nickname-row center `(720,1460)`. After reset, generic summary OCR remains only a page-state gate; the separate `(150,1300,1290,1550)` reader joins left-to-right name tokens and supplies the default nickname used to build the expected result. An empty wide name stops before the second editor/input. Exact width-sensitive editor/final text, CP or a known-baseline CP fallback, exact HP for that fallback, and unchanged distance-8 fingerprint gate persistence. [`BatchScanService`](../../src/pokemon_go_cleanup/batch.py) additionally wraps a verified rename batch only when the switched screenshot's wide nickname equals the first row's saved nickname and its CP/HP/fingerprint match. Tests cover a one-character generic fragment paired with a complete wide default name, wrong editor text, the fixed tap boundary, nickname/CP/HP/fingerprint negatives, durable rows, renamed resume, CP-missing pre-append transition/restoration, and one-swipe fallback. | Source-, synthetic-test-, and saved-real-image-confirmed | The saved `怒鸚哥` frame produces split `怒`/`鸚哥` candidates in two generic variants but the wide reader returns `怒鸚哥`; generic one-character evidence can no longer construct `怒11/7/8`. Real row 1/6 replay also keeps general wrap false while the dedicated wide check returns true for `飄飄球1225` versus expected `飄飄球12/2/5` with identical CP/fingerprint. Real 毛崖蟹 scan `20260827_104456_757116_2a0406bc2bc0430aa724e4c5591635bf` replays from a CP-missing renamed frame as exact nickname `毛崖蟹11/11/15`, exact `96/96HP`, unchanged fingerprint, and inherited baseline CP999. | Automatic and batch walkthroughs |
