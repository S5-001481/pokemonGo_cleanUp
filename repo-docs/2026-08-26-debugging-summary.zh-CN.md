...
最终验证结果为 135 个测试全部通过，Ruff 通过，严格 mypy 检查 19 个源码文件
通过，`git diff --check` 通过。真实截图用于只读回放；最后一项 CP 缺失后的
实际左滑仍需下一次实体设备批量运行确认。
...
- nickname、CP、HP、fingerprint 和页面状态的最终验证没有被局部稳定或快速
  OCR 替代；
- 只有明确的 resume 或 verified-rename 场景可以使用各自专用的 partial/strong
  fallback；
...
- verified-rename CP-missing pre-switch 分支已通过真实截图回放和自动测试，但还需
  一次实体设备多 Pokémon 批量运行确认实际左滑结果；
