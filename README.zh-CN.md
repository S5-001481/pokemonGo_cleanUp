...
等待指定 delay，然后执行左滑：从 `(1180, 1500)` 到 `(260, 1500)`，持续
600 毫秒。之后每 500 毫秒检测一次，最多等待 15 秒；只有页面为
`detail_summary`，且名称、CP 或静态页面指纹至少一项发生变化，才会扫描下一只。
若第一次仍是同一只，在重新确认详情顶部后，只允许再进行一次更明显的左滑：从
`(1300, 1500)` 到 `(140, 1500)`，持续 850 毫秒。程序没有右滑 fallback；页面
状态异常或两次左滑后仍未变化都会安全停止。
